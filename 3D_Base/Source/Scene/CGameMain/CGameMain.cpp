...
void CGameMain::Create()
{
	...
	// ★追加: ボスのBoundingBoxも作成
	m_pBoss->CreateBBoxForMesh(*m_pStaticMeshBBox);
	...
	// ボスの初期位置を設定
		// dynamic_cast を使用して CBoss クラスの InitializeBossPosition を呼び出す
   // ボスの初期位置を設定
	CBoss* bossCast = dynamic_cast<CBoss*>(m_pBoss);
	if (bossCast)
	{
		bossCast->InitializeBossPosition(D3DXVECTOR3(0.f, 0.f, 15.f)); // ボスの初期位置を設定
		// ★重要: ボスにプレイヤーのポインタを設定
		bossCast->SetPlayer(dynamic_cast<CPlayer*>(m_pPlayer));
	}
	else
	{
		OutputDebugStringA("Error: m_pBoss is not a CBoss instance. Cannot call InitializeBossPosition.\n");
	}
}
...
void CGameMain::Update()
{
	...
	//ボスを更新し、そのBoundingBoxも更新
	m_pBoss->Update();
	m_pBoss->UpdateBSpherePos(); // ★追加: ボスのBBoxも更新 (衝突判定前に必要)
	...
	// ボスと地面の当たり判定
	BoundingBox* bossBBox = m_pBoss->GetBBox(); // すでに上で取得済み

	if (bossBBox && groundBBox) // groundBBoxは既に上で取得済み
	{
		if (bossBBox->IsHit(*groundBBox))
		{
			// ★★★ ここでCBoss::GroundCollisionを呼び出す！ ★★★
			currentBoss->HandleGroundCollision(m_pGround);
		}
		// ★重要★
		// もしボスが地面から離れた時にm_GroundedFragをfalseにする必要がある場合
		// (例: プレイヤーのように重力を適用し続ける必要がある場合)
		// ここで衝突していないかをチェックしてfalseに設定するロジックも必要
		// ただし、CBoss::Update() で重力が常に適用され、GroundCollisionでm_GroundedFragがtrueになる設計なら不要
		// (現状のCBoss::Update()ではm_GroundedFragがfalseの時に重力を適用するので、
		//  衝突していない場合はGroundedFragをfalseにしないと、空中にいる間も重力が止まってしまう可能性があります)
		else
		{
			// 地面と衝突していない場合は、接地フラグを下ろす (重力を再開させるため)
			// このロジックはCBoss::Update()の開始時などで行う方がより一般的です。
			// 例えば、Update()の最初にm_GroundedFrag = false; とし、衝突した場合にtrueにする。
		}
	}
...
void CGameMain::Draw()
{
	...
	// ★追加: ボスのBBoxデバッグ表示
	BoundingBox* bossBBox = m_pBoss->GetBBox();
	if (bossBBox) {
		_stprintf_s(dbgText, _T("Boss BBox Min: %.2f, %.2f, %.2f"),
			bossBBox->GetMinPosition().x, bossBBox->GetMinPosition().y, bossBBox->GetMinPosition().z);
		m_pDbgText->Render(dbgText, 10, 110); // 表示位置を調整
		_stprintf_s(dbgText, _T("Boss BBox Max: %.2f, %.2f, %.2f"),
			bossBBox->GetMaxPosition().x, bossBBox->GetMaxPosition().y, bossBBox->GetMaxPosition().z);
		m_pDbgText->Render(dbgText, 10, 130);
	}
}
